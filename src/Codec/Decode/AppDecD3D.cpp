...
/**
*   @brief Function template to decode media file pointed to by szInFilePath parameter.F
...
int NvDecD3D(CUcontext cuContext, char *szInFilePath)
{
	...
	if (nFrame > 0)
	{
		...
		std::cin.get(); // This will keep the application running until you press Enter
	}
